...
- **Evidence:** dialogs.js:885-947; dialogs.js:921 setEditable(false); bridge/commands.ts has no 'editable'/'protected' guard (grep: no matches); dialogs.js:896,907 disabled options. **RUNTIME-CONFIRMED** (probe `restrict-editing-leak.js`, 2026-06-17): under `view.editable===false`, `WC.PM.cmd('toggleBold')` applied bold (0→1 marked nodes, `formatLeaked:true`) → the read-only leak is real. **Already tracked as BUG-005** (see `BUG-LEDGER.md` / `BUGS-DETAILED.md` § BUG-005).
- **Edge cases to test:** Start Enforcing read-only then apply Bold from ribbon (**leaks — confirmed**); read-only then Insert > Table (leaks?); read-only then type in body (DOM blocks); Tracked-changes mode then edit (recorded as tracked + locked); Stop Protection restores editable; no password prompt (Word offers one); Comments/Forms options disabled; reopen pane reflects enforced state
- _needsRuntime: confirmed (format-leak); insert-path still unverified_

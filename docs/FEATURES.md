...
| **Insert** | 10 | 30 | Cover/Blank/Break pages, live-grid Tables (+ Convert Text to Table), Icons as inline SVG, Chart as static image (🟡 interim), Screenshot, Links/Bookmark, Header/Footer/Page Number (plain-text, no galleries), Text Box/Quick Parts/WordArt/Drop Cap, Date & Time, Symbol. Shapes, SmartArt, live Chart, Signature Line, 3D Models, OLE/Excel Spreadsheet, Online Pictures, cloud Add-ins, video playback are ❌ stubs; Cross-ref/Equation/Symbol/Quick Parts are 🟡. See [INSERT-COMPLETENESS.md](bug-hunt/INSERT-COMPLETENESS.md). | [INSERT_TAB.md](INSERT_TAB.md) |

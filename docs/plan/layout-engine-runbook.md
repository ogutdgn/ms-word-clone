...
- [~] **M4** (split into M4a–M4d) — retarget the 6 overlays to the painted per-page DOM. **M4a ✓ + M4b ✓ (+ M4b.1 ✓) + M4c ✓ DONE 2026-06-20** (M4a: comments-ui + track-chrome; M4b/.1: image-resize + live preview; **M4c**: ink-overlay — the SPIKE showed in-app ink ALREADY worked in paged (#pm-editor IS the visible paint host), so M4c fixed the multi-page `.docx` EXPORT: `clientToOverlayLocalPt`/`overlayPageBox` + page-local `posOffset` + re-anchor the node to the draw page + forward-first render page resolution). Remaining: **M4d** notes-area (disable in paged) + header-footer (integration test). **Carry-forwards:** M4b.1 rotated/flipped drag-preview cosmetic (fold into M4d/later if wanted); **M4c** — re-render of a docx-REOPENED multi-page stroke depends on a pre-existing `customGeometry.inkPos`-recovery gap (both modes), out of scope.
...
- **Branch:** `layout-engine` (off `main` @ 7f15724) — **12 commits ahead of origin, local.** M1 + M2 + M3 + M4a + M4b + M4b.1 + M4c ff-merged in (M1–M4b docs archived under `specs/001-paged-render-migration/milestones/`).
- **Done so far:**
...
- **Where we are:** **M1+M2+M3 + M4a + M4b + M4b.1 + M4c done + verified + merged.** Active spec/plan/tasks (M4c) in `specs/001-paged-render-migration/`; M1–M4b archived under `milestones/`. **Side issue (tracked):** `test:pm` shows a pre-existing 268/475 early-abort (stash-verified unrelated to M4c) — investigate separately.
- **NEXT:** **Milestone 4d** (the LAST M4 sub-slice — the LIGHT one): **notes-area = DISABLE in paged** (PE owns footnotes via `semantic-flow-footnotes.ts`; the bridge's notes overlay must no-op/hide in paged so it doesn't double-render or mis-position) + **header-footer = integration TEST** (pure API module, zero coords — just verify it works against PE's `HeaderFooterSessionManager`, no retarget). Run `/speckit-plan` scoped to M4d only → questions → approval → tasks/implement/verify. Then M4 is COMPLETE → M5 (paged `.docx` export COM-oracle parity) + M6 (glyph-metric tolerance).

...
- [~] **M4** (split into M4a–M4d) — retarget the 6 overlays to the painted per-page DOM. **M4a ✓ + M4b ✓ DONE 2026-06-20** (M4a: `posToOverlayLocalY`/`Rect` + comments-ui + track-chrome; M4b: `nodeBoxFor`/`overlayScale` + image-resize, ImageResize installed in the paged PE). Remaining: **M4c** ink-overlay (hard) · **M4d** notes-area (disable in paged) + header-footer (integration test). **Carry-forward (M4b):** live drag-PREVIEW in paged writes to the hidden image node → the painted image snaps to size on release, not during the drag (handles + commit are correct). Fold the painted-element live preview into M4c or an M4b.1.
...
- **Branch:** `layout-engine` (off `main` @ 7f15724). M1 + M2 + M3 + M4a + M4b ff-merged in (M1–M4a docs archived under `specs/001-paged-render-migration/milestones/`).
- **Done so far:**

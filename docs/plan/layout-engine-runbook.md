...
- [~] **M4** (split into M4a–M4d) — retarget the 6 overlays to the painted per-page DOM. **M4a ✓ + M4b ✓ (+ M4b.1 ✓) DONE 2026-06-20** (M4a: `posToOverlayLocalY`/`Rect` + comments-ui + track-chrome; M4b: `nodeBoxFor`/`overlayScale` + image-resize, ImageResize installed in the paged PE; **M4b.1**: live image resize preview on the PAINTED image during drag). Remaining: **M4c** ink-overlay (hard) · **M4d** notes-area (disable in paged) + header-footer (integration test). **Carry-forward — RESOLVED by M4b.1:** the live drag-PREVIEW now writes to the painted image too (onDragMove → `applyPreviewSize(d.paintedDom)`), so the image resizes during the drag, not just on release. **New minor cosmetic (out of 4b's aspect-locked scope):** a ROTATED/flipped image's drag preview can look mis-sized because the painted wrapper's geometry transform isn't recomputed mid-drag (self-corrects on pointer-up) — fold into M4c if wanted.
...
- **Branch:** `layout-engine` (off `main` @ 7f15724) — **9 commits ahead of origin, local.** M1 + M2 + M3 + M4a + M4b + M4b.1 ff-merged in (M1–M4a docs archived under `specs/001-paged-render-migration/milestones/`).
- **Done so far:**
...
- **Where we are:** **M1+M2+M3 + M4a + M4b + M4b.1 done + verified + merged.** Active spec/plan/tasks (M4b) in `specs/001-paged-render-migration/`; M1–M4a archived under `milestones/`.
- **NEXT:** **Milestone 4c** — ink-overlay (the HARD one): retarget `localPt` → a new `WC.PM.coords.clientToOverlayLocalPt` AND the SVG capture surface must track the visible painted page (+ pointer capture during drag); reuse `overlayScale()`. Optionally fold in the rotated-image live-preview cosmetic above. Run `/speckit-plan` scoped to M4c only → questions → approval → tasks/implement/verify. Then M4d (notes-area disable + header-footer test).

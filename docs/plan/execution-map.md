...
- [ ] **NEXT: 012 (frames group)** — the LAST loop feature. The residual `isBlocked` layout controls (position/align/group/rotate of floating objects). Likely NO-FORK (bridge writes to the floating-object/drawing anchor attrs) — SPIKE to confirm. Full spec-kit chain off `general-done`. When 012 merges → ALL 8 done → PushNotification. Then the COMPLETENESS PASS (`docs/bug-hunt/`) as a later phase.

...
- [ ] **NEXT: pick the next spec-kit feature (004+)** — overlay-retirement / residual-`isBlocked`-layout (position/align/group/rotate/lineNumbers/hyphenation/mid-doc-section-breaks) / paged-test-coverage port / M6→gate+pagination-calibration / import-fidelity, then the COMPLETENESS PASS (`docs/bug-hunt/`). Start a fresh `/speckit-specify`.

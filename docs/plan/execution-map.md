...
- [ ] **NEXT: 007 paged test-coverage port** — port the ~70 overlay-only `test:pm` assertions to paged-aware variants (TEST INFRA, no Word oracle — the enabler for 008 overlay retirement; read memory `paged-testpm-overlay-suite`). Then 008→012. Then the COMPLETENESS PASS (`docs/bug-hunt/`) as a later phase.

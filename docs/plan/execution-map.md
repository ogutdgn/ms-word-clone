...
- [ ] **NEXT: 008 overlay retirement** — the genuine paged `test:pm` prerequisite is MET. Fresh sub-branch off `general-done`, full spec-kit chain. ⚠️ **HARD STOP: ask the user BEFORE the destructive overlay-engine deletion.** Then 009→012. Then the COMPLETENESS PASS (`docs/bug-hunt/`) as a later phase.

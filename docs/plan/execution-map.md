...
- [ ] **NEXT: 010 (paged import fidelity)** — fix the gap 007 surfaced: paged `WC.Files.open('*.html')` dumps raw HTML as literal text (the `[7]` PAGED_KNOWN_GAP) + paged txt/csv import. NO-FORK, WC.PM bridge. Fresh sub-branch off `general-done`, full spec-kit chain. Then 011→012. Then the COMPLETENESS PASS (`docs/bug-hunt/`) as a later phase.

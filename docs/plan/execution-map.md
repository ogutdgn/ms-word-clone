...
- [ ] **NEXT: 011 (pagination calibration)** — ⚠️ the STOP-and-ask point. Target the PE-2-vs-Word-3 multi-page page-count divergence (009's known-gap) via the measuring-dom systematic-offset hook. SPIKE FIRST to decide no-fork-reachability; if it needs a fork edit → STOP and ask the user. Then 012 (frames group). Then the COMPLETENESS PASS (`docs/bug-hunt/`) as a later phase.

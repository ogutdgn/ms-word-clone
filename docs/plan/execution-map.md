...
- [ ] **NEXT: 009 (M6 glyph-tolerance → pass/fail GATE)** — turn the dev-box-only `report:glyphgeom` into a gate using `contracts/m6-glyph-tolerance.md` thresholds (the multipage PE-2-vs-Word-3 divergence is 011, not 009). Fresh sub-branch off `general-done`, full spec-kit chain. Then 010→012. Then the COMPLETENESS PASS (`docs/bug-hunt/`) as a later phase.

...
- [ ] **NEXT: 004 P2** (the owned margin-number overlay `line-numbers-overlay.js` — count per mode/countBy/restart over the painted `.superdoc-line`s; tasks T012–T016) → **P3** (Options dialog start/count-by/distance + per-paragraph `w:suppressLineNumbers`, T017–T022). Then 005+ (overlay-retirement / residual-`isBlocked`-layout / paged-test-coverage / M6→gate / import-fidelity), then the COMPLETENESS PASS (`docs/bug-hunt/`).

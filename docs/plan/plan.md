...
| **2** | Editing core behind the existing ribbon (strangler-fig): commands → PM transactions | **in progress (slices 0a–6 done + on `main` via PRs #11–#23 — PM core is the visible editor; file IO + character/paragraph/lists/styles/clipboard+editing-misc/find-replace/insert-basics+full-Table-Tools flipped & oracle-validated; slice 7 file-io next)** |

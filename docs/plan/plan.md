...
| **2** | Editing core behind the existing ribbon (strangler-fig): commands → PM transactions | **in progress (slices 0a–4 done + on `main` — PM core is the visible editor; file IO, character/paragraph/lists/styles/clipboard+editing-misc flipped & oracle-validated; slice 5 find-replace next)** |

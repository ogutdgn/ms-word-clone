...
(async () => {
  ...
  // ---- bug-bust round 1 (high severity) ----
  await t('[bb] Change Case on a multi-paragraph selection keeps paragraph count', () => { E.setHTML('<p>one two</p><p>three four</p>'); const ps = E.node.querySelectorAll('p'); const r = document.createRange(); r.setStart(ps[0].firstChild, 0); r.setEnd(ps[1].firstChild, ps[1].firstChild.length); const s = window.getSelection(); s.removeAllRanges(); s.addRange(r); E.saveRange(); WC.Commands.dropdown({ cmd: 'changeCase', type: 'dropdown' }, document.body); const item = Array.from(document.querySelectorAll('.flyout .fi-label')).find((x) => /UPPERCASE/.test(x.textContent)); if (item) item.click(); document.querySelectorAll('.flyout').forEach((n) => n.remove()); const after = E.node.querySelectorAll('p'); return after.length === 2 && after[0].textContent === 'ONE TWO' && after[1].textContent === 'THREE FOUR'; });
  await t('[bb] Cover page does not stack (insert two -> one)', () => { E.setHTML('<p>body</p>'); WC.Insert.insertCover({ name: 'A', build: () => '<div class="cover-page"><h1>Cover A</h1></div><div class="manual-break"></div>' }); WC.Insert.insertCover({ name: 'B', build: () => '<div class="cover-page"><h1>Cover B</h1></div><div class="manual-break"></div>' }); return E.node.querySelectorAll('.cover-page').length === 1 && /Cover B/.test(E.node.querySelector('.cover-page').textContent); });
  await t('[bb] Update Labels refuses to touch an ordinary user table', () => { E.setHTML('<table><tr><td>my data</td><td>keep</td></tr></table>'); WC.Mail.updateLabels(); return /my data/.test(E.node.innerHTML) && !/Next Record/.test(E.node.innerHTML); });
  await t('[bb] Shift+Tab on a top-level list item makes a valid <p>', () => { E.setHTML('<ul><li>alpha</li><li>beta</li></ul>'); const li = E.node.querySelector('li'); const r = document.createRange(); r.selectNodeContents(li); r.collapse(true); const s = window.getSelection(); s.removeAllRanges(); s.addRange(r); E.saveRange(); E.promoteListItem(); const p = E.node.querySelector('p'); const ok = !!p && p.textContent === 'alpha' && !p.closest('li') && E.node.querySelector('ul') && E.node.querySelector('ul').children.length === 1; return ok; });
  await t('[bb] Design.restore clears stale accents (no --word-blue leak)', () => { WC._themeAccents = null; const snap = WC.Design.snapshot(); WC.setThemeColors(['#ff0000', '#00ff00']); WC.Design.restore(snap); return WC._themeAccents === null; });

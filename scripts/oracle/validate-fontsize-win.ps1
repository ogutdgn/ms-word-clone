# scripts/oracle/validate-fontsize-win.ps1 — read the first paragraph's font size (Font.Size, points)
# from a .docx via a FRESH hidden Word COM instance. PID-safe: spawns its own invisible Word, never
# attaches to the user's; cleans up only what it opened + only the spawned PID. DisplayAlerts=0 → a
# corrupt/repaired file raises a catchable COM error (reported ok=false).
#   powershell -File scripts/oracle/validate-fontsize-win.ps1 <abs path .docx>
# w:sz is HALF-points (20pt → w:sz=40); Word's Font.Size is in points (20). Reads a TEXT-ONLY range
# (excludes the trailing pilcrow, which is unsized → a full Paragraphs(1).Range can read wdUndefined).
$ErrorActionPreference = 'Stop'
try { [Console]::OutputEncoding = [System.Text.Encoding]::UTF8 } catch {}
$path = $args[0]
if (-not $path) { Write-Output '{"error":"usage: validate-fontsize-win.ps1 <abs .docx>"}'; exit 2 }
...
try {
  ...
  if ($doc.Paragraphs.Count -ge 1) {
    ...
    $out.fontSizePt = [double]$r.Font.Size
  }
} catch {
  $out.ok = $false

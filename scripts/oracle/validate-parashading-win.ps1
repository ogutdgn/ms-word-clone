# scripts/oracle/validate-parashading-win.ps1 — read the first paragraph's shading
...
#   powershell -File scripts/oracle/validate-parashading-win.ps1 <abs path .docx>
# Paragraphs(1).Shading.BackgroundPatternColor is a wdColor RGB long (R + G*256 + B*65536); red = 255.
$ErrorActionPreference = 'Stop'
try { [Console]::OutputEncoding = [System.Text.Encoding]::UTF8 } catch {}
$path = $args[0]
if (-not $path) { Write-Output '{"error":"usage: validate-parashading-win.ps1 <abs .docx>"}'; exit 2 }
...
try {
  ...
  if ($doc.Paragraphs.Count -ge 1) {
    $out.backgroundPatternColor = [int]$doc.Paragraphs.Item(1).Shading.BackgroundPatternColor
  }
} catch {
  $out.ok = $false

# scripts/oracle/validate-fontcolor-win.ps1 — read the first paragraph's font color (Font.Color) from a
...
#   powershell -File scripts/oracle/validate-fontcolor-win.ps1 <abs path .docx>
# Font.Color is a wdColor RGB long (R + G*256 + B*65536); pure red FF0000 = 255 (BGR swap would be 16711680).
$ErrorActionPreference = 'Stop'
try { [Console]::OutputEncoding = [System.Text.Encoding]::UTF8 } catch {}
$path = $args[0]
if (-not $path) { Write-Output '{"error":"usage: validate-fontcolor-win.ps1 <abs .docx>"}'; exit 2 }
...
try {
  ...
  if ($doc.Paragraphs.Count -ge 1) {
    # Read a TEXT-ONLY range (exclude the trailing paragraph mark): the pilcrow is uncolored, so a full
    # Paragraphs(1).Range can read as wdUndefined(9999999) for a text-only color. Trim the last char.
    $r = $doc.Paragraphs.Item(1).Range
    if (($r.End - $r.Start) -gt 1) { $r.SetRange($r.Start, $r.End - 1) }
    $out.fontColor = [int]$r.Font.Color
  }
} catch {
  $out.ok = $false

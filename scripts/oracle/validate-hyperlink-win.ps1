# scripts/oracle/validate-hyperlink-win.ps1 — read the first hyperlink (Address / TextToDisplay) from a
...
#   powershell -File scripts/oracle/validate-hyperlink-win.ps1 <abs path .docx>
# doc.Hyperlinks.Item(1).Address resolves only if the External relationship is present in the package.
$ErrorActionPreference = 'Stop'
try { [Console]::OutputEncoding = [System.Text.Encoding]::UTF8 } catch {}
$path = $args[0]
if (-not $path) { Write-Output '{"error":"usage: validate-hyperlink-win.ps1 <abs .docx>"}'; exit 2 }
...
try {
  ...
  $out.ok = $true
  $out.hyperlinkCount = $doc.Hyperlinks.Count
  if ($doc.Hyperlinks.Count -ge 1) {
    $h = $doc.Hyperlinks.Item(1)
    $out.address = [string]$h.Address
    $out.textToDisplay = [string]$h.TextToDisplay
  }
} catch {
  $out.ok = $false

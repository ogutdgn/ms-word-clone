...
## Phase 1: Gate logic
- [x] T001 On `009-glyphgeom-gate` off `general-done`; feature.json → this dir; baseline (the 4 core gates already green).
- [x] T002 `paged-glyphgeom-validate.js`: add the threshold constants (WRAP_MISMATCH_RATE=0, START_X_P95_MAX_PT=1.0,
  LINE_Y_P95_MAX_PT=6.0) sourced from the contract + a documented noise-headroom.
- [x] T003 Compute `gate = { pass, tolerance, checks[], knownGaps[] }` after perFixture/perFontSummary: single-page
...
## Phase 2: npm + comment
- [x] T004 `package.json`: add `test:glyphgeom` = `node scripts/paged-glyphgeom-validate.js` (alias of report:glyphgeom).
...
- [x] T005 Run `npm run report:glyphgeom` (or `test:glyphgeom`) on the dev box → gate PASSES (SC-001) + the multipage
  page-count appears as a ⚠️ known-gap (SC-003). Confirm enumVerified/fontVerified/scaleVerified/pidSafe still hold.
- [x] T006 SC-002: temporarily tighten a threshold (e.g. LINE_Y_P95_MAX_PT=0.1) → confirm exit 1 + the breach is
  reported; REVERT.
- [x] T007 Confirm the 4 core gates unaffected (test:pm 416 / smoke 9 / roundtrip 27 / test:bundle 4) — they don't run
  the glyphgeom path, so a quick spot-check (no src change) suffices.

## Phase 4: Review + close-out
- [x] T008 `/code-review` (adversarial-review workflow) — focus: does the gate ACTUALLY assert (not just report); is
...
- [x] T009 Docs: a one-line note (glyphgeom is now a gate; the multipage page-count divergence is tracked to 011).
  Tick this tasks.md.
- [x] T010 ff-merge `009-glyphgeom-gate` → `general-done` (NOT main, no push). plan-tracking checkpoint.

...
- [x] T017 [US3] Added `suppressLineNumbers(on?)` to `src/renderer/bridge/line-numbers.ts` (spike-chosen no-fork: `editor.commands.updateAttributes('paragraph', { 'paragraphProperties.suppressLineNumbers': true|null })` set/clear/toggle) + `currentParagraphSuppressed()`; merged onto `WC.PM` + no-op fallback.
- [x] T018 [US3] Added the "Line Numbering Options" dialog in `commands.js` (`WC.dialog`: numbering mode / start-at / count-by / from-text distance) → `WC.PM.setLineNumbers(...)`; wired "Suppress for Current Paragraph" → `WC.PM.suppressLineNumbers()` (toggle + ✓). Seeded from `getLineNumbers()`. No `E()` in the handler. start/countBy USER-FACING (bridge maps the w:start off-by-one); distance sent only when entered.
- [x] T019 [US3] Extended `scripts/paged-linenumbers-probe.js`: start 5 → export RAW `w:start="4"` (=userStart−1, off-by-one) + `getLineNumbers().start===5`; count-by 2 → `w:countBy="2"`; distance 0.5 → `w:distance="720"` + `distanceExplicit`, none ⇒ Auto; suppress → exactly one bare `<w:suppressLineNumbers/>` in the marker `pPr` (no leak) + null-clear; overlay SKIPS all lines of the suppressed paragraph (count drops). (NB: spec.md AS-1 "`w:start="5"`" is stale — off-by-one writes raw 4.)
- [x] T020 [US3] Extended `scripts/oracle/validate-linenumbers-win.ps1`: reads `.CountBy`/`.StartingNumber`/`.DistanceFromText` + the marker paragraph's `w:suppressLineNumbers` (Range.WordOpenXML, ON-not-`w:val=0`); extended the roundtrip C6 (StartingNumber===5 + suppressMarkerFound + paragraphSuppressed).
